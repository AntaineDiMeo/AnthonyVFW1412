...
NSString * DiMeo_Anthony_Project2part1$ModuleRequireFormat = @"(function(exports){"
...
@implementation DiMeo_Anthony_Project2part1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dimeo_anthony_project2part1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_dimeo_anthony_project2part1 gc];
}
...
{
	// create DiMeo_Anthony_Project2part1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "DiMeo_Anthony_Project2part1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dimeo_anthony_project2part1 = [[DiMeo_Anthony_Project2part1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dimeo_anthony_project2part1];
	
	NSString *_dimeo_anthony_project2part1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dimeo_anthony_project2part1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dimeo_anthony_project2part1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:DiMeo_Anthony_Project2part1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dimeo_anthony_project2part1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.dimeo_anthony_project2part1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
