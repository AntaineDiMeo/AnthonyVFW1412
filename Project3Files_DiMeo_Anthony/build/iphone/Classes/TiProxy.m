...
//Rare exceptions to indicate a bug in the _project3files code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_project3files = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_project3files]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.project3files.module"
