...
//Rare exceptions to indicate a bug in the _views code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_views = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_views]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.views.module"
