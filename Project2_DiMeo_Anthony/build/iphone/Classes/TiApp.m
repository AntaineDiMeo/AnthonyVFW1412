...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kProject2_DiMeo_AnthonyUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kProject2_DiMeo_AnthonyUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Project2_DiMeo_Anthony app using the node.js CLI
