...
//Rare exceptions to indicate a bug in the _project2_dimeo_anthony code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_project2_dimeo_anthony = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_project2_dimeo_anthony]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.project2_dimeo_anthony.module"
