...
NSString * Takehome2_DiMeo_Anthony$ModuleRequireFormat = @"(function(exports){"
...
@implementation Takehome2_DiMeo_AnthonyObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_takehome2_dimeo_anthony);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Takehome2_DiMeo_Anthony global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Takehome2_DiMeo_Anthony" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_takehome2_dimeo_anthony = [[Takehome2_DiMeo_AnthonyObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_takehome2_dimeo_anthony];
	
	NSString *_takehome2_dimeo_anthonyNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _takehome2_dimeo_anthonyNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_takehome2_dimeo_anthony valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Takehome2_DiMeo_Anthony$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.takehome2_dimeo_anthony.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.takehome2_dimeo_anthony.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
