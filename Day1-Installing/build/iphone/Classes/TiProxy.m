...
//Rare exceptions to indicate a bug in the _day1_installing code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_day1_installing = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_day1_installing]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.day1_installing.module"
