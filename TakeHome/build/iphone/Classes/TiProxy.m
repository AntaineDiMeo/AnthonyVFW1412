...
//Rare exceptions to indicate a bug in the _takehome code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_takehome = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_takehome]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.takehome.module"
