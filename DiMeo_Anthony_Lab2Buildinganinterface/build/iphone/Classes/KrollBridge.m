...
NSString * DiMeo_Anthony_Lab2Buildinganinterface$ModuleRequireFormat = @"(function(exports){"
...
@implementation DiMeo_Anthony_Lab2BuildinganinterfaceObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_dimeo_anthony_lab2buildinganinterface);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_dimeo_anthony_lab2buildinganinterface gc];
}
...
{
	// create DiMeo_Anthony_Lab2Buildinganinterface global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "DiMeo_Anthony_Lab2Buildinganinterface" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_dimeo_anthony_lab2buildinganinterface = [[DiMeo_Anthony_Lab2BuildinganinterfaceObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_dimeo_anthony_lab2buildinganinterface];
	
	NSString *_dimeo_anthony_lab2buildinganinterfaceNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _dimeo_anthony_lab2buildinganinterfaceNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_dimeo_anthony_lab2buildinganinterface valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:DiMeo_Anthony_Lab2Buildinganinterface$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.dimeo_anthony_lab2buildinganinterface.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.dimeo_anthony_lab2buildinganinterface.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
