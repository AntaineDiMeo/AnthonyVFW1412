...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kPractical_DiMeo_AnthonyUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kPractical_DiMeo_AnthonyUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Practical_DiMeo_Anthony app using the node.js CLI
